...
rm -rvf log.txt  
#mvn clean -Dmaven.test.skip=true jetty:run -o -Pdevel,kemt >log.txt |less log.txt
ps ax|grep jetty
#~/bin/perl/killProgram.pl jetty &
mvn clean -Dmaven.test.skip=true jetty:run -o -Pdevel,kemt 
