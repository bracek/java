...
public class EditMessageController {
    ...
    /** The logger. */
    private static final Log logger = LogFactory.getLog(EditMessageController.class);
...
// ----------------------------- CONSTRUCTORS ------------------------------
    ...
    public String handle(@RequestParam(value = ApplicationCodes.MESSAGE.COMMAND, required = false) Integer id, ModelMap model, HttpServletRequest request) throws Exception {
        logger.debug("handle()[GET]: has just been started. Given message id " + id);
    ...
    public String handle(@ModelAttribute(ApplicationCodes.MESSAGE.COMMAND) Message message, BindingResult result, HttpServletRequest request) throws Exception {
        logger.debug("handle()[POST]: has just been started. Given product is " + message);
