...
public class ImportUsersWindow extends Window {
    ...
    public void loadLdapUsers(Event event) throws Exception {
        ...
        AuthenticationWindow win = (AuthenticationWindow) Executions.createComponents("/WEB-INF/jsp/tiles/admin/authPage.zul", null, new HashMap<String, Object>());
        win.doModal();
        ...
        if (!win.isCanceled()) {
            ...
            if (ldapName != null && ldapPassword != null) {
                ...
                try {
                	ldapList = ldapClient.getUsersFullNameList("OU=Kosice,OU=Foreign,OU=People", getLdapURL(),
                			"simple", "CN=" + ldapName + ",OU=Kosice,OU=Foreign,OU=People,DC=ixonos,DC=local", ldapPassword);
                } catch (Exception e) {
                	Messagebox.show("Could not connect to LDAP", "Error", Messagebox.OK, Messagebox.ERROR);
                	return;
                }
                ...
                Messagebox.show("Wrong or empty name or password", "Error", Messagebox.OK, Messagebox.ERROR);
            }
        }
    }
    ...
    public void importLdapUsers(Event event) throws Exception {
        ...
        if (selectedUsers.size() <= 0) {
            Messagebox.show("No record selected", "Error", Messagebox.OK, Messagebox.ERROR);
            return;
        }
        ...
        for (Object listItem : listItems) {
            ...
            try {
            	userList = ldapClient.findUsersByCN(userCNLabel, getLdapURL(), "simple", "CN=" + ldapName + ",OU=Kosice,OU=Foreign,OU=People,DC=ixonos,DC=local", ldapPassword);
            } catch (Exception e) {
            	Messagebox.show("Could not connect to LDAP", "Error", Messagebox.OK, Messagebox.ERROR);
            	break;
            }
            ...
            if (userList.size() <= 0) {
                ...
                Messagebox.show("User not found for search criteria '" + userCNLabel + "'", "Error", Messagebox.OK, Messagebox.ERROR);
                ...
                try {
                    usersService.addNewUser(account, password, name, surname, email, phone, null, null, true, null, Arrays.asList(new String[]{ROLE_USER}));
                    ...
                    mailService.sendNewAccountCreatedEmail(new InternetAddress(email, name + " " + surname), textParameters);

                    i++;

                } catch (Exception e) {
                    logger.error("#importLdapUsers: Error on adding new user", e);
                    Messagebox.show("User " + name + " " + surname + " already exists in database.", "Info", Messagebox.OK, Messagebox.INFORMATION);
                }
            }
        }

        Messagebox.show("Number of successfully added users into database: " + i, "Info", Messagebox.OK, Messagebox.INFORMATION);
    ...
    private String getLdapURL() {
        return "ldap://" + configService.getStringProperty(LDAP_HOST, LDAP_HOST_DEFAULT) + ":" + configService.getIntProperty(LDAP_PORT, LDAP_PORT_DEFAULT) + "/" + configService.getStringProperty(LDAP_BASE_DN, LDAP_BASE_DN_DEFAULT);
