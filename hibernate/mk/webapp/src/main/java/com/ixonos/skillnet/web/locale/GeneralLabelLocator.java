...
public class GeneralLabelLocator implements LabelLocator { 
	...
	*/ 
	public URL locate(Locale locale) throws Exception { 
		...
		String path = "com/ixonos/skillnet/web/bundle/labels/";
		URL url = SkillnetInitServlet.class.getClassLoader().getResource(path + filename);
		if (url == null){
			url = SkillnetInitServlet.class.getClassLoader().getResource(path + defaultFilename);
		}
