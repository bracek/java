		...
		<div class="form-buttons">
			<div class="button">
				<input type="submit" id="save" name="_eventId_save"
					...
					onclick="Spring.remoting.submitForm('save', 'person')" />&#160; <input
...
<c:if test="${not empty person.id}">
	<div style="clear: both; float: left;">
		<div>
			<a href="${flowExecutionUrl}&_eventId=addAddress"><fmt:message
	...
	<table class="search">
		...
		<c:forEach var="address" items="${person.addresses}">
			<tr>
				...
				<td><a
					href="${flowExecutionUrl}&_eventId=editAddress&addressId=${address.id}"><fmt:message
							key="button.edit" /></a> <sec:authorize ifAllGranted="ROLE_ADMIN">
						<a
							href="${flowExecutionUrl}&_eventId=deleteAddress&addressId=${address.id}"><fmt:message
								key="button.delete" /></a>
					</sec:authorize></td>
			</tr>
		</c:forEach>
